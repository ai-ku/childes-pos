...
my $usage = q{Usage: framerun.pl runid seed data_name frame_type};
...
#print STDERR "$runid $seed $data $frame $tmp";
...
my $runmike = "mike_childes -f $mike_data -i $minfo[2] -o $minfo[3] -seed $seed > $mike_out 2> $mike_err";
#print STDERR "$runmike\n";
system($runmike);

my @res = split(' ', `cat $mike_out`);
$tm = time - $tm;
print join("\t", $runid, $seed, $frame, $res[0], $tm)."\n";
