...
my $usage = q{Usage: splitwordsubrun.pl  runid seed foldid nsub iters fold data-name};
...
$tm = time - $tm;
for(my $i = 0 ; $i < @data; $i++){
    print join("\t", $runid, $seed, $foldId, $nsub, $data[$i], $res[$i], $tm, $iter, $minfo[0], $minfo[1], $minfo[2], $trsizes[$i])."\n";
}
